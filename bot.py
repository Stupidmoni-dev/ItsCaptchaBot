...
async def _new_members(msg: Message):
    # import json
    # print(json.dumps(msg.json, ensure_ascii=False, indent=4))  # debug: print full message json
...
async def _kick(bot: AsyncTeleBot, db):
    ...
    async for chat_id, user_id, message_id in db.iter_users_to_kick():
        try:
            # ban user temporarily (31 sec = minimum)
...
async def start():
    ...
    # init DB and create captcha table if not exist
    await db.bootstrap()
    await db.sql("""
    ...
    # scheduler to check captcha timeouts every 10 sec
    scheduler = AsyncIOScheduler()
    scheduler.add_job(_kick, 'interval', seconds=10, args=[bot, db])
    scheduler.start()

    # set bot default admin rights (delete and restrict)
    ...
    # close DB connection when bot stops
    await db.teardown()
